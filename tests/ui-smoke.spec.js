...
test("signal, validation, and character flows render without console errors", async ({ page }) => {
  ...
  await expect(page.locator("[data-leaderboard-market]")).toHaveCount(0);
  await expect(page.locator(".leaderboard-scope-note")).toContainText("통합 선택 적용 중");
  const japanMarketTab = page.locator('[data-stats-market="jp"]');
  await japanMarketTab.click();
  await expect(japanMarketTab).toHaveAttribute("aria-selected", "true");
  await expect(page.locator(".leaderboard-scope-note")).toContainText("日本 선택 적용 중");
