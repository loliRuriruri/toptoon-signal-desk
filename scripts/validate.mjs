...
const recordsWithMotion = records.filter((record) => record.safe_video_url);
assert(recordsWithMotion.length >= records.length * 0.95, "at least 95% of locale records should have a validated official motion URL");
assert(recordsWithMotion.every((record) => /^https:\/\/showcase\.chat\.(?:toptoon\.(?:com|jp|net)|global\.toptoon\.com)\/(?:character\/\d+\/video-thumbnail\/[a-z0-9-]+|pmaker\/translation\/[a-z0-9_/-]+)\.mp4$/i.test(record.safe_video_url)), "every present motion URL should be a validated official motion URL");
